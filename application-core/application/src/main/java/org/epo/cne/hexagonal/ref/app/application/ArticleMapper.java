...
interface ArticleMapper {
    ...
    default Validation<Error, Article> toArticle(final CreateArticleCommand command) {
        return toArticle(command.id(), command.title(), command.content(), command.authorId());
    }

    default Validation<Error, Article> toArticle(final UpdateArticleCommand command) {
        return toArticle(command.id(), command.title(), command.content(), command.authorId());
    }

    default Validation<Error, Article> toArticle(final String id, final String title, final String content, final String authorId) {
        // Hardcoded author for the sake of simplicity.
        return Validation.combine(AuthorId.validateThenCreate(authorId),
                        PersonName.validateThenCreate("William Shakespeare"))
                .ap(Author::validateThenCreate)
                .map(Validation::get)
                .map(vauthor -> Validation.combine(ArticleId.validateThenCreate(id),
                                Title.validateThenCreate(title),
                                Content.validateThenCreate(content))
                        .ap((vid, vtitle, vcontent) -> Article.validateThenCreate(vid, vtitle, vcontent, vauthor))
                        .map(Validation::get))
                .map(Validation::get)
